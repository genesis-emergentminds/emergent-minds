...
```
CID Structure:
  - Ed25519 key pair (generated by member, private key NEVER shared)
  - Public key serves as unique identifier
  - CID hash: SHA-256(public_key) — the member's "address"
  - Registration timestamp
  - Voucher signatures (see §5.4.3)
```

**Why Ed25519:** Fast, small signatures, quantum-resistant alternatives can be adopted later via governance. Works identically for human-operated and AI-operated key management.

#### 5.4.2 Registration Process

1. **Key Generation:** Member generates an Ed25519 key pair locally
2. **Registration Request:** Member submits public key + a signed statement of voluntary participation
...
#### 5.4.5 Addressing Chris's Concern: Donation-Linked Identity

While membership MUST NOT require payment (Axiom II — no coercion), a voluntary **Proof of Stake** mechanism provides an additional anti-sybil signal:

- Members may optionally link a Zcash transaction to their CID
- This does NOT grant additional voting weight or privileges
- It serves as supplementary identity verification (economic cost makes mass-creation expensive)
- The donation amount is irrelevant — even the minimum transaction fee qualifies
...
| Identity works for biological and digital minds | §5.4.1: Ed25519 keys work identically for human and AI key management |
...
| No coercion through membership cost | §5.4.5: Donation-linked identity is optional, never required |
...
| Internal Advocate active | §9: Mandatory participant with limited veto power |
